...
### **1. Что такое относительная скорость?**
...
V^{AB} = 8 - 5 = 3 \, \text{м/с}.
$$
Это означает, что объект $B$ движется на $3 \, \text{м/с}$ быстрее, чем объект $A$.

---

### **2. Почему мы используем нормаль столкновения?**
...
Мы хотим выразить относительную скорость $V^{AB}$ вдоль направления нормали $n$. Для этого используется **скалярное произведение**.

---

### **3. Что такое скалярное произведение?**
Скалярное произведение — это математическая операция, которая позволяет найти проекцию одного вектора на другой. В нашем случае это поможет нам найти компоненту относительной скорости $V^{AB}$, направленную вдоль нормали $n$.
...
- Если $V^{AB} = [3, 4]$ и $n = [0.6, 0.8]$, то:
$$
V^{AB} \cdot n = 3 \cdot 0.6 + 4 \cdot 0.8 = 1.8 + 3.2 = 5
$$

Это означает, что проекция относительной скорости на нормаль $n$ равна $5$.

---

### **4. Что такое реституция?**
...
### **Итоговый алгоритм:**
1. Вычислите относительную скорость $V^{AB} = V^B - V^A$.
2. Найдите проекцию $V^{AB}$ на нормаль $n$:
   $$
   v_{\text{before}} = V^{AB} \cdot n
   $$
3. Определите коэффициент реституции $e = \min(A.\text{restitution}, B.\text{restitution})$.
4. Примените закон Ньютона:
   $$
   v_{\text{after}} = -e \cdot v_{\text{before}}
   $$
